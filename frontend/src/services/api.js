...
export const examsAPI = {
  ...
  submitAnswer: (attemptId, data) =>
    api.post(`/exams/attempts/${attemptId}/answer/`, data),
...
export const consultationsAPI = {
  ...
  selectConsultant: (consultantId) =>
    api.post("/consultations/select/", { consultant_id: consultantId }),
  getMyConsultations: () => api.get("/consultations/my-consultations/"),
  getSchedules: (consultationId) =>
    api.get(`/consultations/${consultationId}/schedules/`),
  createSchedule: (consultationId, data) =>
    api.post(`/consultations/${consultationId}/schedules/`, data),
  getMessages: (consultationId) =>
    api.get(`/consultations/${consultationId}/messages/`),
  sendMessage: (consultationId, message) => {
    return api.post(`/consultations/${consultationId}/messages/`, {
      consultation: consultationId, // این خط رو اضافه کن
      message: message,
      content: message, // اینم اضافه کن شاید این رو میخواد
    });
  },
  endConsultation: (consultationId) =>
    api.post(`/consultations/${consultationId}/end/`),
  activateConsultation: (consultationId) => {
    return api.patch(`/consultations/${consultationId}/activate/`);
    // یا ممکنه از put استفاده بشه:
    // return api.put(`/consultations/${consultationId}/`, { status: 'active' })
  },
};
...
export const notesAPI = {
  list: () => api.get("/notes/"), // همین رو نگه دار  getDetail: (id) => api.get(`/notes/${id}/`),
  upload: (formData) =>
...
// این خط رو اضافه کن تا در کنسول در دسترس باشه
